...
const { simpleStrategy, hiLoCountingStrategy, bookStrategy } = require('./strategies');
...
const playShoe = (numDecks, playerStrategy, dealerStrategy, countingStrategy) => {
    ...
    while (decks.length >= cutIndex) {
        ...
        runningCount += dealerHands[0].hands.reduce((count, card) => count + countingStrategy({ card }), 0)
        for (let i = 0; i < playerHands.length; i++) {
            runningWin += evaluateHands({ playerHand: playerHands[i], dealerHand: dealerHands[0] });
            runningCount += playerHands[i].hands.reduce((count, card) => count += countingStrategy({ card }), 0)
        }
...
const testPlayHand = () => {
    ...
    const { hands, endingCount } = playHand({ hands: [], dealerFaceUp, decks, strategy: bookStrategy });
    console.log('Dealer', dealerFaceUp)
    console.log('Player', hands.map(ph => ph.hands))
